...
const initMap = () => {
    ...
    // Centrer sur le lieu associé
    const initialLat = props.enigme.lieu?.latitude || 48.8566;
    const initialLng = props.enigme.lieu?.longitude || 2.3522;
    ...
    if (props.enigme.lieu?.latitude && props.enigme.lieu?.longitude) {
        marker = L.marker([props.enigme.lieu.latitude, props.enigme.lieu.longitude]).addTo(map);
    }
};
